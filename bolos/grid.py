...
class Grid(ABC):
    ...
    def __init__(self, x0: float, x1: float, n: int) -> None:
        self.x0 = np.array(x0)
        self.x1 = np.array(x1)
    ...
    def interpolate(self, f: np.ndarray, other: 'Grid') -> np.ndarray:
        ...
        if self._interp is None:
            # Should we extrapolate linearly instead of by closest value?
    ...
    def cell(self, x: float) -> int:
        ...
        """
        return int((self.f(np.array(x)) - self.fx0) / self.df)
    ...
    @abstractmethod
    def finv(self, w: np.ndarray) -> np.ndarray:
        raise NotImplementedError()
...
class GeometricGrid(Grid):
    ...
    here the length of cell i+1 is r times the length of cell i.
    """

    def __init__(self, x0: float, x1: float, n: int, r: float = 1.1) -> None:
        self.r = r
        self.logr = np.log(r)
        self.rn_minus_1 = np.exp(n * self.logr) - 1

        super().__init__(x0, x1, n)
...
class LogGrid(Grid):
    ...
    def __init__(self, x0: float, x1: float, n: int, s: float = 10.) -> None:
        self.s = s
        super().__init__(x0, x1, n)
